...
def authorize():
    ...
    user_level, full_name = authorize_user(name, pw)
    if 0 <= user_level <= 4:
        access_token = generate_access_token(name, user_level)
        message = ""
    return set_cors({
...
def get_contents():
    ...
    try:
        ...
        user_level = -1
        expected_token = generate_access_token(name, user_level)
...
def generate_access_token(name, user_level):
    """ Create a session token. """
    # Use standard approaches (like JWT) in your future projects.
    today = datetime.now().strftime("%Y-%m")
    message = (name + str(user_level) + today).encode()
